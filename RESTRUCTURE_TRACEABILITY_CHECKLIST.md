...
- [~] `App.__init__` / `_set_taskbar_icon` / `_build_ui` (136) / `_on_resize` / `_shake_window` / `destroy` → `adapters/driving/ui_tkinter/app_shell.py` — *partial:* `__init__` now wires the facade seam (`self._dispatcher = TkEventDispatcher`, `self._workspace = WorkspaceSession`, `self.core_api = build_tk_core_api(...)`) and instantiates the extracted views; AppShell relocation itself is the final M5 step.
...
- [x] `App` hotkeys: `_register_hotkeys` / `_unregister_hotkeys` / `_hk_run` / `_hk_stop` / `_hk_space` → `adapters/driving/ui_tkinter/hotkey_adapter.HotkeyAdapter` — **M5 DONE**: global OS hotkeys (only place touching `keyboard`) bound to injected run/stop/pause intents, marshalled via the dispatcher (run debounce preserved). `App.__init__` builds + registers it; `destroy` unregisters. No `_hk_*`/`_last_run_hk` left in baru.
...
- [~] `App` monitors/zones: `_refresh_monitors` / `_on_screen_selected` / `_find_monitor_by_info` / `_capture_monitor_thumbnail` / `_draw_minimap` / `_open_overlay` / `_overlay_done` / `_save_zones` / `_load_zones` / `_update_overlay_btn` → `adapters/driving/ui_tkinter/views/` (zone/monitor views; data via facade) — *partial:* the `ScreenSelectorPanel` widget relocated to `adapters/driving/ui_tkinter/components/screen_selector.py` (self-contained; `baru` re-exports as shim). App monitor/zone methods + the import cluster (`ImportView`, big — Excel/metadata-DB) still pending.
- [~] `App` mode/run: `_set_mode` / `_update_mode_buttons` / `_on_mode_change` / `_run_test` / `_stop_test` / `_test_finished` / `_toggle_pause` / `_toggle_suite` / `set_execution_state` (45) / `_cb_progress` / `_cb_paused` / `_cb_done` / `_on_auto_paused` → `adapters/driving/ui_tkinter/views/run_controls.py` — *partial:* **`RunControls` view DONE** (owns the 6 Run/Pause/Stop buttons + `set_run_state`/`set_run_enabled`; clicks → injected intents; state-transition-verified). `set_execution_state` now delegates the run buttons to it and keeps only the non-run toolbar buttons. Also **`ModeView` DONE** (the 3 operating-mode buttons + active-highlight; `_update_mode_buttons` delegates to `mode_view.set_active`; `_set_mode`/`_on_mode_change` keep the orchestration + `run_mode` StringVar + `btn_load_excel` toggle). Remaining: run *methods* still drive `SuiteRunner` directly (facade `start_suite` routing deferred).
- [x] `App._settings_dialog` (124) → `adapters/driving/ui_tkinter/views/settings_view.py` — **M5 DONE**: thin forwarder builds `SettingsView(self, self.core_api, …)`; the view reads/writes via `api.get_config`/`update_config` (no `APP_CONFIG`/globals). `App._on_settings_applied` keeps only side effects (legacy globals, `grid_spacing`, OCR re-init, refresh). e2e-verified.
...
- [x] `App._log` (7) → `adapters/driving/ui_tkinter/views/log_sink.py` — **M5 DONE**: `LogSink` owns the log text panel + render; `_log` is a one-line delegation shim (deletion ticket: a `LogMessage` event). Marshals via the dispatcher.
- [x] `App` stats: `_refresh` / `_refresh_stats_only` / `_update_stats` → `adapters/driving/ui_tkinter/views/stats_view.py` — **M5 DONE (render)**: all 9 stat labels (incl. the run-state label) live in `StatsView`; 24 `lbl_*.config` pokes collapsed to `stats.set(field,…)`. `_update_stats` etc. stay in `App` and drive the view. *(Also: `RunProgressView` owns the progress bar + status label — 10 pokes → `run_progress.set_fraction/set_text`.)*
...
- [~] `OverlayWindow` (32 methods) → **split (R-HEX-3)** — *started:* `_canvas_to_abs`/`_abs_to_canvas` maths extracted to **`adapters/driving/ui_tkinter/components/canvas_viewport.CanvasViewport`** ✅ (pure, origin-provider, unit-tested `tests/test_canvas_viewport.py`; the two methods are now thin delegators). **Still pending:** zone ops + undo (`_save_zone_type`/`_delete_zone`/`_undo`/`_push_undo`/`_check_zone_size`/`_change_zone_type`) → `core/domain/zone.py` (`ZoneLayout`/`ZoneEditSession`); drawing/interaction/toolbar → `components/zone_overlay.py`; `_link_zone_to_anchor` → `core/services/` (anchor).
