...
class SuiteCompleted(Event):
    ...
    passed: int = 0
    failed: int = 0
    # Payload for report generation (P2.3 cutover). Subscribers that only care
    # about pass/fail can ignore these. A report subscriber sets
    # `report_generated = True` so the engine knows the report was handled and
    # need not fall back to a direct call.
...
class CardStarted(Event):
    ...
    # Recorder payload (B3): a subscriber starts the per-card recorder from these
    # and sets recorder_handled so the runner won't also start one (no double-rec).
    scenario: Optional[Any] = None
    evidence_dir: Optional[Any] = None
    recorder_handled: bool = False
