...
# M2.1: flow step enums + dynamic proc-type relocated to core/domain/flow.py;
# re-exported here as shims (existing `iscs_workflow.ProcedureType` etc. unchanged).
from core.domain.flow import (
    ProcedureCategory, ProcedureStatus, ProcedureType,
    _DynamicProcType, _resolve_proc_type,
)


# ═════════════════════════════════════════════════════════════════════════════
#  CAPABILITY REGISTRY BRIDGE + ENGINE  ->  relocated to core/services/engine.py (M3.4)
# ═════════════════════════════════════════════════════════════════════════════
# Re-exported as shims so dialogs / baru / plugins / tests are unchanged. The
# dispatcher reads `core_registry` from the engine module; this wf-level alias is
# what the (still-here) `_dynamic_catalogue` palette reads.
from core.services.engine import (
    _LEGACY_METHOD_MAP, _category_for, _noop_log, _to_step_status,
    LegacyExecContext, LegacyCapabilityAdapter, register_legacy_capabilities,
    registry_step_coverage,
    _CORE_OK, core_registry, core_bus,
...
# M2.1: flow containers relocated to core/domain. iscs_workflow re-exports them
# as shims so the engine / dialogs / plugins / tests are unchanged.
from core.domain.flow import (
    Procedure, IOGroup, ProcedureFlow,
    FLOW_SCHEMA_VERSION, _FLOW_MIGRATORS, register_flow_migrator, _migrate_flow_dict,
...
# M3.3: auto_register_procedures relocated to core/services/import_service.py; shim.
from core.services.import_service import auto_register_procedures


# ═════════════════════════════════════════════════════════════════════════════
#  SHARED EXECUTION CONTEXT + PROCEDURE RUNNER  ->  core/services/engine.py (M3.4)
# ═════════════════════════════════════════════════════════════════════════════
from core.services.engine import ExecContext, _StandaloneCtx, ProcedureRunner
...
# M3.4: build_runner_from_scenario relocated to core/services/engine.py; shim.
