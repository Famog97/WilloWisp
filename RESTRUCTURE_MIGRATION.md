...
| [x] | M3.5 | Application Services | Extract workspace session profile state (closes `_clear_workspace`). *(`WorkspaceSession` owns the zone working set (`zones` + `zones_per_page`); `baru.App` delegates via property getter/setter, so a new UI shares the same core state. `_clear_workspace` state-reset → `WorkspaceSession.clear()`.)* | `core/services/workspace.py` |
...
| 🔄 | M5.2 | App Shell & Layout | Extract window geometry, resizing, layouts, and views from legacy `baru.py`. *(**Out so far:** `LogSink` ✅, `RunProgressView` ✅, `StatsView` ✅, `RunControls` ✅, `ModeView` ✅; component `ScreenSelectorPanel` ✅, `CanvasViewport` ✅ (R-HEX-3), `HotkeyAdapter` ✅ — Strangler, one at a time; `App` still instantiates them. AppShell itself pending.)* | `adapters/driving/ui_tkinter/app_shell.py` · `adapters/driving/ui_tkinter/views/` |
| 🔄 | M5.3 | Intent Forwarders | Convert Run, Import, Settings, and Card Config controls into thin facade-forwarders. *(**`App` holds `core_api`.** `SettingsView` ✅ (live config). `RunControls` ✅ — Run/Pause/Stop cluster + the `set_execution_state` run-button state machine; buttons forward to injected intents, run-state drives them (state-transition-verified). Run *methods* (`_run_test`…) still orchestrate `SuiteRunner` directly — facade `start_suite` routing deferred. Import / Card-Config pending.)* | `adapters/driving/ui_tkinter/views/` |
...
| 🔄 | M5.5 | Overlay Coordinate Split | Split drawing canvas overlays from underlying geometry data (R-HEX-3). *(Started: `CanvasViewport` ✅ — pure canvas↔absolute maths extracted from `OverlayWindow` + unit-tested. Pending: `ZoneEditSession` (zone ops+undo → `core/domain/zone.py`) and the `zone_overlay.py` drawing component.)* | `adapters/driving/ui_tkinter/components/` |
